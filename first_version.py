...
class NN:
    def __init__(self, input_data, actual_results):
        ...
        self.weights_1 = np.array(pd.read_csv("/Users/vladikturin/Desktop/pycharm_projects/tytanic/weights1.csv"))
        self.weights_2 = np.array(pd.read_csv("/Users/vladikturin/Desktop/pycharm_projects/tytanic/weights2.csv"))
...
# The best result was when I took 33 examples from the sample
    def bp_gradient_descent(self, epochs, alpha, lmbda):
        for i in range(epochs):
            ...
            sys.stdout.flush()
            for j in range(0, self.hypothesis.shape[0], 1):
                self.delta_weights_1 = np.zeros(np.transpose(self.weights_1).shape)
                self.delta_weights_2 = np.zeros(np.transpose(self.weights_2).shape)

                # Find the error on unsigmoid data
                output_error = self.hypothesis[j:j+1] - self.actual_results[j:j+1]

                # Find the error in the hidden layer
                _ = np.matmul(output_error, np.transpose(self.weights_2))
                hidden_layer_error = _ * sigmoid_derivative(add_ones(self.hidden_layer[j:j+1]))
                ...
                self.delta_weights_2 = self.delta_weights_2 + np.matmul(np.transpose(output_error), self.hidden_layer_activation[j:j + 1]) + lmbda * np.transpose(self.weights_2) / self.hypothesis.shape[0]
                self.delta_weights_1 = self.delta_weights_1 + np.matmul(np.transpose(hidden_layer_error[:, 1:]), self.input_data[j:j + 1]) + lmbda * np.transpose(self.weights_1) / self.hypothesis.shape[0]

                self.weights_2 -= np.transpose(self.delta_weights_2) * alpha
                self.weights_1 -= np.transpose(self.delta_weights_1) * alpha
                # sys.stdout.write(f"Batch: {j}, error: {logistic_cost(self.hypothesis, self.actual_results)}\n")


if __name__ == "__main__":
    b = pd.read_csv("/Users/vladikturin/Desktop/pycharm_projects/tytanic/data.csv")
    ...
    a.bp_gradient_descent(10000, 0.0005, 0.4)
